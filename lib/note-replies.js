...
export function noteCommentText(comment = {}) {
  ...
  const fromJson = textFromBodyJson(comment.body_json || comment.bodyJson).replace(/\s+/g, " ").trim();
  return fromJson;
}
...
function parentIdFromAncestorPath(path) {
  const parts = String(path || "")
    .replace(/^\.+|\.+$/g, "")
    .split(".")
    .filter((part) => /^\d+$/.test(part));
  return parts.length ? parts.at(-1) : null;
}
...
function normalizeBranch(branch) {
  ...
  for (const raw of descendantsRaw) {
    ...
    const pathParent = parentIdFromAncestorPath(raw.ancestor_path || raw.ancestorPath);
    const parentId = stringId(explicitParent || pathParent || root.id);
    byId.set(stringId(raw.id), normalizeComment(raw, parentId, 0));
  }
...
export function buildNoteThread(comments, selectedCommentId, articleUrl) {
  ...
  const byId = new Map((comments || []).map((comment) => [stringId(comment.id), { ...comment, id: stringId(comment.id), parentId: comment.parentId ? stringId(comment.parentId) : null }]));
  ...
  while (queue.length) {
    ...
    for (const comment of byId.values()) {
      if (comment.parentId === parent && !descendantIds.has(comment.id)) {
  ...
  const included = (comments || []).filter((comment) => ancestorIds.has(stringId(comment.id)) || descendantIds.has(stringId(comment.id)));
  const baseDepth = Math.min(...included.map((comment) => Number(comment.depth) || 0));
